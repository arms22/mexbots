...
def mylogic(ticker, ohlcv, position, balance, strategy):
    ...
    sell_qty = int(spot_limit * spot_qty / (ticker.ask * 0.0002))
    buy_qty = int(spot_limit * spot_qty / (ticker.bid * 0.0002))
    long_size = max(position.currentQty,0)
    short_size = max(-position.currentQty,0)

    logger.info(f'{F_pct:.4f}/{P_pct:.4f} {F8_pct:.4f}/{P8_pct:.4f} {sell_qty} {buy_qty} {spot_limit:.8f}')
